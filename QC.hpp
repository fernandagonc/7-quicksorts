...
void particaoQC(int esq, int dir, int *i, int *j, int vetor[], int *n_comp, int *n_mov){
    *i = esq; *j = dir;
    int pivo = vetor[(*i + *j)/2]; //define o pivo de acordo com a especificação do trabalho para o quicksort tradicional
    int aux;
...
void QC(int esq, int dir, int vetor[], int *n_comp, int *n_mov){ 
    int i, j;
    particaoQC(esq, dir, &i, &j, vetor, n_comp, n_mov); //particionamos o vetor

    if(esq < j) 
        QC(esq, j, vetor, n_comp, n_mov);
    if(i < dir) 
        QC(i, dir, vetor, n_comp, n_mov);
